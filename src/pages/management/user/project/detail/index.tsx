...
export default function ProjectPage() {
  ...
  const isInterviewed = (proposalId: string) => interviewData.some(
    (interview) => interview.proposal === proposalId
  );
  ...
  const getColumnSearchProps = (dataIndex: keyof ProposalInfo): any => ({
    ...
    filterIcon: (filtered: boolean) => (
      <Icon icon="mingcute:search-2-line" className={filtered ? "text-primary" : ""} />
    ),
    onFilter: (value: string, record: ProposalInfo) =>
      (record[dataIndex] || "")
        .toString()
        .toLowerCase()
  ...
  const columns: ColumnsType<ProposalInfo> = [
    ...
    },
    {
      ...
      filterDropdown: ({
        ...
        const value = Array.isArray(selectedKeys[0])
      ...
      onFilter: (value: Key, record: ProposalInfo) => {
        ...
        return (
          recordDate.isAfter(dayjs(start)) &&
          recordDate.isBefore(dayjs(end))
        );
      },
      width: 50,
