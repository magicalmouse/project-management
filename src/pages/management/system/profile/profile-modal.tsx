...
export default function ProfileModal({ title, show, formValue, onOk, onCancel }: ProfileModalProps) {
	...
	const onSubmit = async (values: ProfileInfo) => {
		...
		const profile = title === "New Profile" ? { ...dataWithoutId, user: normalizedUser } : { ...values, user: normalizedUser };
						...
						<FormField
							...
							render={({ field }) => (
								<FormItem>
									...
									<FormControl>
										<Select value={typeof field.value === "string" ? field.value : field.value.id} onValueChange={(value) => field.onChange(value)}>
